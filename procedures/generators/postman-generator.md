...
# postman-generator — Procedure (canonical)

*Canonical definition. Generic across any Slim-style PHP API project with a central route map. The deployed copy at `procedures/generators/postman-generator.md` solves the requirements below with project-specific values.*
...
Task  : #postman-generator
Model : claude-sonnet-4-6
Type  : Generator
Output: one `{output-dir}/{ControllerClassName}.postman_collection.json` per discovered controller — flush-and-fill across the full set.

*Test-mode output goes to `automation/tests/procedures/postman-generator/{ControllerClassName}.postman_collection.json` (one file per controller). The `--test` flag and test-mode redirect are defined in `_generator_base`.*

---

## Requirements

*Declared generically here; solved in the deployed copy. See `_generator_base` for pre-flight resolution and the fail rule.*
...
Read `{route-source}`. Identify every distinct controller class referenced in route group registrations. These are the controllers that have routes and need collections.
...
If an output file already exists for a controller: read it and extract `info._postman_id` for reuse. Otherwise generate a new UUID for that controller.
...
**Collection-level variables** — always include `{base-url-var}` and `{auth-var}`; add one variable per unique path param appearing in this controller's routes.
...
**Multiple-surface rule** — when a route accepts meaningfully different input states, generate one named sibling request per variation. Split only on genuinely different call patterns, not different data values.

**Saved examples** — include at least one saved example per request; one per explicitly-returned status code for routes with multiple outcomes.
...
List any unmatched routes (routes in `{route-source}` with no corresponding method) at the bottom of the report.
