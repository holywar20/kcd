...
Every investigator supports the `--test` flag.

Under `--test`, output is redirected to `_Claude/automation/tests/procedures/{Name}.md` — the procedure's normal audit file is never touched. Every phase runs identically; only the destination changes. The procedure creates `automation/tests/procedures/` if absent. Flush-and-fill.

`--test` exists to exercise a procedure unattended and inspect its findings without producing real product.
