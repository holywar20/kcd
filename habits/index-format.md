...
*Inherited by all lenses via _base. Defines the canonical shape of any navigational index in the project.*

---

**When:** writing, modifying, or auditing any index file.

An "index" is any file whose primary purpose is navigation — pointing readers at other files. Examples in this project: `plans/index.md`, `kcd/habits/index.md`, the `## Know` and `## Do` reference tables in every lens, the catalogue at `references/domain/widget-catalogue.md`. If a file's job is to answer "where do I find X?" it's an index.
...
- **What** — the destination's name in plain language. Not the filename. ("Per-Widget API Surface", not "dd_per-widget-api-surface".)
- **Where** — a markdown link whose link text is the file stem (or a meaningful short slug). The href is the relative path.
- **Why** — one phrase capturing what the reader gets by clicking through. Bias toward purpose ("decompose persistence into per-widget endpoints") over content summary ("contains 6 phases and a table"). One sentence max.
...
**Why this format:** the *why* column is the load-bearing one. Without it, an index is just a directory listing — the reader has to open files to find what they need. With it, the reader can scan, decide, and click once. This is the difference between "I have a folder of plans" and "I have a navigable plan library." It compounds as the index grows.
